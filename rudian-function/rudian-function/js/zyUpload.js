		...
		return this.each(function() {
			...
			this.createHtml = function() {
				...
				if (para.dragDrop) {
					...
					html += '							<input type="hidden" name="packets" id="packetsv"><input type="hidden" name="sign" id="signv">';
					html += '						</div>';
					html += '					</div>';
					...
					html += '						<div class="upload_btn">开始上传</div><div class="cancel_upload">关闭</div>';
					html += '					</div>';
					html += '				</div>';
				...
				var params = {
					...
					onSuccess: function(file, res) {
						...
						if (returnCode === "000") {
							...
							$('<div class="tu-p" style="background:url(http://106.3.37.173:81/image/' + returnMsg[0].path + ') no-repeat center;background-size:contain;" data-url="http://106.3.37.173:81/image/' + returnMsg[0].path + '" data-id="' + returnMsg[0].id + '"><img src="http://106.3.37.173:81/image/' + returnMsg[0].path + '" style="display:none;"/><span class="xt-s"></span></div>').insertAfter($('.jia-t'));
						}
